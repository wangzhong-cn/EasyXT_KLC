...
class RiskAnalyzer:
    ...
    def _calculate_max_drawdown(self, values: list[float]) -> tuple[float, int]:
        ...
        # 最大回撤
        max_dd: float = float(np.min(drawdown))
    ...
    def _calculate_var(self, returns: list[float], confidence_level: float = 0.05) -> float:
        ...
        returns_array = np.array(returns)
        var = float(np.percentile(returns_array, confidence_level * 100))
    ...
    def _calculate_profit_factor(self, returns: list[float]) -> float:
        ...
        gross_profit: float = float(np.sum(returns_array[returns_array > 0]))
        gross_loss: float = float(abs(np.sum(returns_array[returns_array < 0])))
