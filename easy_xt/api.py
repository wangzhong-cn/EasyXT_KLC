...
from .trade_api import TradeAPI, OrderResponse
...
class EasyXT:
    ...
    def buy(self,
        ...
        Args:
            ...
            signal_id: 策略信号ID（用于审计）

        Returns:
            OrderResponse: 委托响应结果结构，失败时 order_id 为 None
        """
        if not self._trade_connected:
            ErrorHandler.log_error("交易服务未初始化")
            return OrderResponse(None, "error", "交易服务未初始化")
        return self.trade.buy(account_id, code, volume, price, price_type, signal_id)
    ...
    def sell(self,
        ...
        if not self._trade_connected:
            ErrorHandler.log_error("交易服务未初始化")
            return OrderResponse(None, "error", "交易服务未初始化")
        return self.trade.sell(account_id, code, volume, price, price_type, signal_id)
