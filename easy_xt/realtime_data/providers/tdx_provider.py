...
class TdxDataProvider(BaseDataProvider):
    ...
    def __init__(self, config: Optional[dict[str, Any]] = None):
        ...
        self.timeout = self.config.get('timeout', 3)  # Reduce default timeout to 3s
        self.retry_count = self.config.get('retry_count', 1)  # Fix 62c: 降为1次，避免重连风暴
        self.retry_delay = self.config.get('retry_delay', 1)
        self.error_log_cooldown = self._resolve_error_log_cooldown(self.config)
        self._error_log_last_ts: dict[str, float] = {}
        self._error_log_suppressed: dict[str, int] = {}

    @staticmethod
    def _resolve_error_log_cooldown(config: dict[str, Any]) -> float:
        env_value = os.environ.get("EASYXT_TDX_ERROR_LOG_COOLDOWN")
        raw = env_value if env_value is not None else config.get("error_log_cooldown", 15.0)
        try:
            return max(float(raw), 0.0)
        except Exception:
            return 15.0

    def _log_throttled(self, level: str, key: str, message: str) -> None:
        now = time.monotonic()
        cooldown = max(float(self.error_log_cooldown), 0.0)
        last = self._error_log_last_ts.get(key)
        if last is not None and cooldown > 0 and now - last < cooldown:
            self._error_log_suppressed[key] = int(self._error_log_suppressed.get(key, 0)) + 1
            return
        suppressed = int(self._error_log_suppressed.pop(key, 0))
        log_func = getattr(self.logger, str(level).lower(), self.logger.error)
        if suppressed > 0:
    ...
    def connect(self) -> bool:
        ...
        # Fix 62: 最多尝试 3 台服务器，避免全部超时累计 >30s 阻塞后台线程
        max_attempts = min(3, len(self.servers))
        servers = self.servers[:max_attempts]

        for server in servers:
            try:
                self.logger.info(f"尝试连接服务器: {server['host']}:{server['port']}")
                # Fix 62: 每次 connect 前重建 API 实例，确保 socket 超时生效
                self.api = TdxHq_API()
                ...
                if result:
                    ...
                    self._error_log_last_ts.clear()
                    self._error_log_suppressed.clear()
                    return True
                else:
                    self._log_throttled(
                        "warning",
                        f"connect_fail:{server['host']}:{server['port']}",
                        f"连接失败: {server['host']}:{server['port']}",
                    )

            except Exception as e:
                self._log_throttled(
                    "error",
                    f"connect_exc:{server['host']}:{server['port']}",
                    f"连接异常: {server['host']}:{server['port']}, 错误: {e}",
                )
                continue
            ...
            try:
                ...
                for i in range(0, len(stock_list), batch_size):
                    ...
                    try:
                        ...
                        if quotes:
                            for quote in cast(list[dict[str, Any]], quotes):
    ...
    def _format_quote_data(self, quote: dict) -> Optional[dict[str, Any]]:
        ...
        try:
            ...
            return {
                ...
                'bid1_vol': int(quote.get('bid_vol1', 0) or quote.get('bid1_vol', 0)),
                'ask1_vol': int(quote.get('ask_vol1', 0) or quote.get('ask1_vol', 0)),
                'bid2': float(quote.get('bid2', 0)),
                'ask2': float(quote.get('ask2', 0)),
                'bid2_vol': int(quote.get('bid_vol2', 0) or quote.get('bid2_vol', 0)),
                'ask2_vol': int(quote.get('ask_vol2', 0) or quote.get('ask2_vol', 0)),
                'bid3': float(quote.get('bid3', 0)),
                'ask3': float(quote.get('ask3', 0)),
                'bid3_vol': int(quote.get('bid_vol3', 0) or quote.get('bid3_vol', 0)),
                'ask3_vol': int(quote.get('ask_vol3', 0) or quote.get('ask3_vol', 0)),
                'bid4': float(quote.get('bid4', 0)),
                'ask4': float(quote.get('ask4', 0)),
                'bid4_vol': int(quote.get('bid_vol4', 0) or quote.get('bid4_vol', 0)),
                'ask4_vol': int(quote.get('ask_vol4', 0) or quote.get('ask4_vol', 0)),
                'bid5': float(quote.get('bid5', 0)),
                'ask5': float(quote.get('ask5', 0)),
                'bid5_vol': int(quote.get('bid_vol5', 0) or quote.get('bid5_vol', 0)),
                'ask5_vol': int(quote.get('ask_vol5', 0) or quote.get('ask5_vol', 0)),
    ...
    def get_minute_data(self, code: str, count: int = 240) -> list[dict[str, Any]]:
        ...
        try:
            ...
            # 获取分时数据
            data = self.api.get_minute_time_data(market, std_code)

            if not data:
                return []

            formatted_data = []
            minute_items = cast(list[dict[str, Any]], data)
            if count > 0:
                minute_items = minute_items[-count:]
    ...
    def get_market_status(self) -> dict[str, Any]:
        ...
        try:
            ...
            sz_count_val = int(sz_count) if isinstance(sz_count, (int, float)) else 0
            sh_count_val = int(sh_count) if isinstance(sh_count, (int, float)) else 0

            return {
                'sz_market_count': sz_count_val,
                'sh_market_count': sh_count_val,
                'total_count': sz_count_val + sh_count_val,
