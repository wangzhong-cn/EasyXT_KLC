...
class TaskScheduler:
    ...
    async def _process_scheduled_tasks(self):
        """处理定时任务"""
        current_time = datetime.now(tz=_SH)
    ...
    async def _run_task_wrapper(self, task: Task):
        ...
        try:
            ...
            task.result.end_time = datetime.now(tz=_SH)
            ...
            if task.result.retry_count < task.max_retries:
                ...
                task.scheduled_time = datetime.now(tz=_SH) + timedelta(seconds=retry_delay)
    ...
    def schedule_task(self,
        ...
        if isinstance(scheduled_time, (int, float)):
            scheduled_time = datetime.now(tz=_SH) + timedelta(seconds=scheduled_time)
    ...
    def schedule_recurring_task(self,
        ...
        recurring_info = {
            ...
            'execution_count': 0,
            'next_execution': datetime.now(tz=_SH) + timedelta(seconds=interval)
        }
        ...
        def recurring_wrapper():
            try:
                ...
                if recurring_task_id in self.recurring_tasks:
                    next_time = datetime.now(tz=_SH) + timedelta(seconds=recurring_info['interval'])
                    self.schedule_task(
