...
class ErrorHandler:
            ...
            if recovery_action.condition(error_info):
                try:
                    ...
                    if result:
                        ...
                        error_info.resolution_time = datetime.now(tz=_SH)
    ...
    def _circuit_breaker_action(self, error_info: ErrorInfo) -> Any:
        ...
        if circuit_breaker.state == CircuitBreakerState.OPEN:
            if circuit_breaker.next_attempt_time and datetime.now(tz=_SH) >= circuit_breaker.next_attempt_time:
        ...
        circuit_breaker.last_failure_time = datetime.now(tz=_SH)

        if circuit_breaker.failure_count >= circuit_breaker.failure_threshold:
            circuit_breaker.state = CircuitBreakerState.OPEN
            circuit_breaker.next_attempt_time = datetime.now(tz=_SH) + timedelta(seconds=circuit_breaker.recovery_timeout)
    ...
    def get_errors(self,
        ...
        if hours:
            cutoff_time = datetime.now(tz=_SH) - timedelta(hours=hours)
    ...
    def get_error_statistics(self) -> dict[str, Any]:
        ...
        # 最近24小时错误
        recent_cutoff = datetime.now(tz=_SH) - timedelta(hours=24)
    ...
    def _cleanup_old_errors(self):
        ...
        # 按时间清理
        cutoff_time = datetime.now(tz=_SH) - timedelta(hours=cleanup_hours)
