...
class RealtimeDataAPIServer:
    ...
    def __init__(self, config: RealtimeDataConfig):
        ...
        self.stats: dict[str, Any] = {
            'start_time': datetime.now(tz=_SH),
            'total_requests': 0,
    ...
    def _setup_routes(self):
        ...
        # 实时行情数据
        app.router.add_get('/api/v1/quotes', self.get_quotes)
        app.router.add_post('/api/v1/quotes', self.get_quotes_batch)

        # 热门股票
        app.router.add_get('/api/v1/hot-stocks', self.get_hot_stocks)

        # 概念数据
        app.router.add_get('/api/v1/concepts', self.get_concepts)

        # 市场状态
        app.router.add_get('/api/v1/market-status', self.get_market_status)

        # 数据源状态
        app.router.add_get('/api/v1/sources', self.get_sources_status)

        # 多数据源对比
        app.router.add_get('/api/v1/compare', self.compare_sources)

        # 服务器统计
        app.router.add_get('/api/v1/stats', self.get_server_stats)

        # API文档
        app.router.add_get('/docs', self.get_api_docs)
        app.router.add_get('/', self.get_api_docs)  # 根路径重定向到文档
    ...
    async def _logging_middleware(self, request: web_request.Request, handler):
        ...
        try:
            ...
            duration = (datetime.now(tz=_SH) - start_time).total_seconds()
    ...
    async def get_status(self, request: web_request.Request) -> web_response.Response:
        ...
        return web.json_response({
            'server': {
                ...
                'uptime_seconds': (datetime.now(tz=_SH) - self.stats['start_time']).total_seconds(),
                'host': self.host,
                'port': self.port
            },
