...
if __name__ == "__main__":
    ...
    # 因子3：中度相关
    factor_dict['alpha003'] = factor_dict['alpha001'] * 0.6 + np.random.randn(len(dates), len(stocks)) * 0.4

    # 因子4：独立因子
    factor_dict['alpha004'] = pd.DataFrame(
        np.random.randn(len(dates), len(stocks)),
        index=dates,
        columns=stocks
    )
    ...
    # 因子5：与因子4高度相关
    factor_dict['alpha005'] = factor_dict['alpha004'] * 0.85 + np.random.randn(len(dates), len(stocks)) * 0.15
    ...
    # 保存结果
    analyzer.save_correlation_matrix('correlation_matrix.csv')
    analyzer.save_report('correlation_report.csv')
