...
class SimpleCache:
    ...
    def get(self, key: str, ttl: int = 300) -> Optional[Any]:
        ...
        if datetime.now(tz=_SH) > entry['expires']:
            del self.cache[key]
            return None
    ...
    def set(self, key: str, value: Any, ttl: int = 300) -> None:
        ...
        self.cache[key] = {
            'value': value,
            'created': datetime.now(tz=_SH),
            'expires': datetime.now(tz=_SH) + timedelta(seconds=ttl)
        }
...
def retry(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            raise RuntimeError(f"函数 {func.__name__} 未知重试状态")
