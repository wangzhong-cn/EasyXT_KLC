"""
专门处理月份开始逻辑的转换器
将run_weekly和run_monthly转换为run_daily并添加月份判断逻辑
"""
import re
from typing import Dict, List

class JQToPtradeMonthlyConverter:
    """处理月份开始逻辑的转换器"""
...
def initialize(context):
    ...
    # [回测版本已禁用] check_limit_up 需要实时行情，回测环境中无法使用
...
def is_month_start(context):
    """
    判断是否为月份开始
    由于Ptrade没有run_monthly，需要在run_daily中判断
    """
    import datetime
    ...
    # 简化处理，实际应该获取当月所有交易日并取第一个
    first_trade_day = 1  # 这里简化处理，实际应该调用get_trade_days
...
def weekly_adjustment(context):
    ...
    # 判断是否为周一或者特定条件
    # 由于Ptrade没有run_weekly，需要在函数内部判断
    import datetime
...
'''
    ...
    def _extract_functions(self, jq_code: str) -> Dict[str, str]:
        ...
        key_functions = [
            ...
            'is_month_start', 'month_start'  # 特别处理月份开始函数
        ]
...
def initialize(context):
    ...
    run_monthly(month_start, time='9:05', reference_security='000300.XSHG')

def month_start(context):
    # 回测当前时间获取
    ...
    # 获取该月份的所有交易日
    trade_days = get_trade_days(start_date=str(year)+"-" + str(month)+'-01', 
                               end_date=str(year)+"-" + str(month)+'-20', 
                               count=None)
    
    # 返回第一个交易日
    first_trade_day = trade_days[0]
    trade_year, trade_month, trade_day = first_trade_day.split('-')
    ...
    start_flag = 0
    if month == context.current_dt.month and day == trade_day:
        start_flag = 1
...
'''
    ...
    # 创建转换器
    converter = JQToPtradeMonthlyConverter()
