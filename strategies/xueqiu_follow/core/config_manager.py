...
class ConfigManager:
        ...
        try:
            ...
            if unified_config_path.exists():
                ...
                if xueqiu_config_path.exists():
                    with open(xueqiu_config_path, encoding='utf-8') as f:
    ...
    def _load_portfolios(self):
        ...
        try:
            ...
            if self.portfolios_file.exists():
                with open(self.portfolios_file, encoding='utf-8') as f:
    ...
    def get_enabled_portfolios(self) -> list[dict[str, Any]]:
        """获取启用的组合配置"""
        with self._lock:
            portfolios_obj: Any = self._portfolios
            # 处理两种格式：字典格式和列表格式
            # 首先检查是否是portfolios.json格式：{"portfolios": [...], "global_settings": {...}}
            if isinstance(portfolios_obj, dict) and 'portfolios' in portfolios_obj:
                # portfolios.json 格式
                portfolios_list = portfolios_obj.get('portfolios', [])
                return [p for p in portfolios_list if isinstance(p, dict) and p.get('enabled', False)]
            elif isinstance(portfolios_obj, dict):
                # 如果是字典格式（从default.json加载的组合配置）
                enabled_portfolios = []
                for code, portfolio_config in portfolios_obj.items():
            ...
            elif isinstance(portfolios_obj, list):
                return [p for p in portfolios_obj if isinstance(p, dict) and p.get('enabled', False)]
            else:
                return []
    ...
    def validate_settings(self) -> list[str]:
        ...
        # 验证账户配置
        qmt_path_raw = self.get_setting('account.qmt_path', '')
        qmt_path = str(qmt_path_raw) if qmt_path_raw is not None else ''
